...
- HPバー: `boss_hpbar.png` をゆっくり回転させて装飾、下地に円弧で残HP表示。12時から時計回りに欠ける（方向反転済み）。HP残量で色変化: 赤→橙→黄
- 魔法陣: 射撃中のみボス背面に表示、時計回り回転。大技中は拡大＋逆回転の二重魔法陣
- 通常4フェーズ（360Fで循環）
  - Phase 0: 自機狙い扇5発 + sine移動
  - Phase 1: 全方位回転星弾2層（逆回転） + 中央固定
  - Phase 2: ランダム散らし星弾 + dashワープ移動
  - Phase 3: way5(緑中弾) + 自機狙い3発 + figure-8移動
- 大技（スペルカード）2種: HP70%で「紅蓮双輪」（中央固定、虹色逆回転2層リング＋自機狙いバースト）、HP35%で「朱紅爆裂」（ワープ移動＋放射爆発＋回転リング）。発動時は画面の敵弾を一掃＋光波紋エフェクト、約9秒持続
